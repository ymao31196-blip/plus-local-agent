...
function Stop-OwnedListener([int]$Port, [string[]]$RequiredFragments, [string]$Name) {
    ...
    foreach ($fragment in $RequiredFragments) {
        if (-not $commandLine.Contains($fragment)) {
            throw "$Name port $Port belongs to PID $pidValue, but it is not the expected PLA process. Refusing to stop it.`nCommandLine: $commandLine"
        }
    }
...
# Stop the tunnel first so it does not keep forwarding while the local runtime shuts down.
